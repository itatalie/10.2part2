import pytest
from src.generators import filter_by_currency, generate_transaction_descriptions, generate_card_numbers
...
def test_filter_by_currency(sample_transactions, currency, expected_count):
    ...
    assert len(result) == expected_count, f"Неверное количество транзакций для валюты {currency}"
...
def test_generate_transaction_descriptions(sample_transactions):
    ...
    assert len(descriptions) == len(sample_transactions), "Количество описаний не совпадает с количеством транзакций"
...
def test_generate_card_numbers(start, stop, expected_numbers):
    ...
    for card_number in result:
        ...
        assert len(card_number.replace(" ", "")) == 16, "Номер карты должен содержать 16 цифр без пробелов"