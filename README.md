...
- `read_csv_transactions(file_path)`: Считывает данные из CSV-файла.
- `read_excel_transactions(file_path)`: Считывает данные из Excel-файла.

## Новая функциональность

- Добавлена возможность поиска транзакций по описанию с использованием регулярных выражений.
- Добавлена функция подсчета количества операций по категориям.
- Реализован интерфейс для взаимодействия с пользователем через модуль `main`.